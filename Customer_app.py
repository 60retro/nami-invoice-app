...
# --------------------------------------------------------
# ส่วนที่ 2: ค้นหาที่อยู่ด้วยรหัสไปรษณีย์ (ปรับปรุงใหม่: แสดงตัวเลือกทันที)
# --------------------------------------------------------
...
# Logic การค้นหาที่อยู่ (ทำงานทันทีเมื่อครบ 5 หลัก หรือกดปุ่ม)
if (len(input_zip) == 5 and not thai_db.empty) or btn_zip:
    if len(input_zip) == 5:
        # แปลงเป็น string เพื่อความชัวร์ในการค้นหา
        ...
        if not results.empty:
            ...
            # --- 🔴 จุดที่แก้ไข: ใช้ st.radio แทน st.selectbox ---
            st.info(f"👇 พบ {len(options)} พื้นที่ (กรุณาจิ้มเลือกรายการที่ถูกต้อง)")
            
            # แสดงตัวเลือกแบบกางออกมาเลย (Radio)
            selected_option = st.radio(
                "เลือกพื้นที่:", 
                options, 
                index=0,  # เลือกตัวแรกให้เลย เพื่อความเร็ว
                label_visibility="collapsed" # ซ่อนหัวข้อซ้ำซ้อน
            )
            ...
            if selected_option:
                ...
                # st.success("✅ เลือกเรียบร้อย!") # ไม่ต้องแสดงก็ได้ เพื่อความสะอาดตา
