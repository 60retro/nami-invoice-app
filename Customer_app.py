...
# --------------------------------------------------------
# ส่วนที่ 3: กรอกรายละเอียด (ปรับปรุงใหม่: รองรับชื่อสาขาที่เป็นตัวหนังสือ)
# --------------------------------------------------------
st.header("3️⃣ ตรวจสอบข้อมูลให้ครบถ้วน")

# --- 🟢 ส่วนที่ปรับปรุงใหม่: ชื่อ และ ประเภทสาขา (รองรับ Text) ---
...
# เปลี่ยนข้อความตัวเลือกที่ 2 ให้สื่อความหมายชัดเจนขึ้น
...
if branch_type == "สาขา (ระบุเลข หรือ ชื่อสาขา)":
    # ปรับเป็น Text Input ธรรมดา ไม่จำกัดจำนวนตัวอักษร
    branch_input = st.text_input("ระบุเลขสาขา หรือ ชื่อสาขา", placeholder="เช่น 00001 หรือ บางนา (ไม่ต้องพิมพ์คำว่าสาขา)")
    
    if branch_input:
        # กันลูกค้าพิมพ์คำว่า "สาขา" มาซ้ำ เช่น พิมพ์ว่า "สาขาบางนา" -> ตัดออกเหลือ "บางนา"
        clean_branch_name = branch_input.replace("สาขา", "").strip() 
        # ประกอบร่างใหม่ให้ถูกต้องเสมอ
...
if st.button("✅ ยืนยันข้อมูลถูกต้อง (กดตรงนี้)", type="primary", use_container_width=True):
    ...
    if not c_name_raw or not c_tax:
        ...
        st.error("❌ กรุณาเลือก 'ประเภทหน่วยงาน'")
    elif branch_type == "สาขา (ระบุเลข หรือ ชื่อสาขา)" and not branch_input: # เช็คว่ากรอกช่องสาขาไหม
