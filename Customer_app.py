...
# --------------------------------------------------------
# ส่วนที่ 2: ค้นหาที่อยู่ด้วยรหัสไปรษณีย์ (Selectbox ใน Expander)
# --------------------------------------------------------
...
if (len(input_zip) == 5 and not thai_db.empty) or btn_zip:
    if len(input_zip) == 5:
        ...
        if not results.empty:
            ...
            # --- 🔴 จุดที่แก้ไข: ใช้ st.expander ครอบ st.selectbox ---
            # expanded=True จะทำให้กรอบนี้ "เด้ง" กางออกมาทันที เรียกร้องความสนใจ
            
            with st.expander(f"✅ พบ {len(options)} พื้นที่ (กรุณากดเลือกแขวงและเขตของคุณในกล่องด้านล่าง)", expanded=True):
                # ใช้ Selectbox เหมือนเดิม (ประหยัดที่) แต่มีกรอบช่วยเน้น
                selected_option = st.selectbox(
                    "กดที่นี่เพื่อเลือกตำบล/อำเภอ:", 
                    options, 
                    index=0, 
                    label_visibility="visible" # แสดงหัวข้อให้ชัด
                )
...
# --------------------------------------------------------
# ส่วนที่ 3: กรอกรายละเอียด (ปรับปรุงใหม่: เพิ่มตัวเลือกสาขา)
# --------------------------------------------------------
st.header("3️⃣ ตรวจสอบข้อมูลให้ครบถ้วน")

# --- 🟢 ส่วนที่ปรับปรุงใหม่: ชื่อ และ ประเภทสาขา ---
...
# ตัวเลือกประเภทสาขา (บังคับเลือก)
branch_type = st.radio(
    "เลือกประเภทหน่วยงาน (เพื่อเติมท้ายชื่อให้ถูกต้อง):",
    options=["(สำนักงานใหญ่)", "สาขา (ระบุเลข)", "บุคคลธรรมดา (ไม่เติมท้ายชื่อ)"],
    index=None,  # บังคับให้ผู้ใช้กดเลือกเอง (ไม่มีค่า Default) เพื่อกันลืม
    horizontal=True
)

branch_suffix = "" # ตัวแปรเก็บข้อความที่จะไปต่อท้ายชื่อ

if branch_type == "สาขา (ระบุเลข)":
    # ถ้าเลือกสาขา -> แสดงช่องกรอกเลขสาขาเพิ่ม
    branch_no = st.text_input("ระบุเลขสาขา (เช่น 00001)", max_chars=10, placeholder="ใส่ตัวเลขรหัสสาขา")
    if branch_no:
        branch_suffix = f" (สาขา {branch_no})"
elif branch_type == "(สำนักงานใหญ่)":
    branch_suffix = " (สำนักงานใหญ่)"
elif branch_type == "บุคคลธรรมดา (ไม่เติมท้ายชื่อ)":
    branch_suffix = "" # ไม่เติมอะไรเลย

# แสดงตัวอย่างชื่อเต็มให้เห็นชัดๆ
if c_name_raw:
...
if st.button("✅ ยืนยันข้อมูลถูกต้อง (กดตรงนี้)", type="primary", use_container_width=True):
    # --- 🔴 เพิ่ม Validation ตรวจสอบสาขา ---
    if not c_name_raw or not c_tax:
        st.error("❌ กรุณากรอก 'ชื่อ' และ 'เลขผู้เสียภาษี'")
    elif branch_type is None: # ถ้ายังไม่เลือกประเภทสาขา
        st.error("❌ กรุณาเลือก 'ประเภทหน่วยงาน' (สำนักงานใหญ่ / สาขา / บุคคลธรรมดา)")
    elif branch_type == "สาขา (ระบุเลข)" and not branch_no: # เลือกสาขาแต่ไม่กรอกเลข
        st.error("❌ กรุณาระบุ 'เลขสาขา'")
    # ------------------------------------
    elif len(c_tax) != 13:
        ...
        if st.session_state['last_submitted_id'] == sig:
            ...
            try:
                ...
                # ใช้ c_name_final แทน c_name เดิม
                ...
                try:
                    ...
                    if need_save:
                        # ใช้ c_name_final ที่มีสาขาต่อท้ายแล้ว
