...
# 🛡️ ส่วนตรวจสอบ Tax ID (Validator)
c_tax = st.text_input("เลขประจำตัวผู้เสียภาษี (ต้องมี 13 หลัก)", value=search_taxid, max_chars=13)

# แจ้งเตือน Real-time ถ้าเลขไม่ครบ
if c_tax and len(c_tax) < 13:
    st.warning(f"⚠️ เลขผู้เสียภาษียังไม่ครบ 13 หลัก (ตอนนี้มี {len(c_tax)} หลัก)")
if c_tax and not c_tax.isdigit():
...
# ==========================================
# 🔘 ปุ่มยืนยัน (พร้อมระบบตรวจสอบก่อนส่ง)
# ==========================================
st.markdown("")
if st.button("✅ ยืนยันข้อมูล (กดเพียงครั้งเดียว)", type="primary", use_container_width=True):
    ...
    # 🛡️ Check 2: ความถูกต้องของ Tax ID
    elif len(c_tax) != 13:
        st.error(f"❌ เลขผู้เสียภาษีต้องมี 13 หลัก (คุณกรอกมา {len(c_tax)} หลัก)")
    elif not c_tax.isdigit():
        st.error("❌ เลขผู้เสียภาษีต้องเป็นตัวเลขเท่านั้น")
    # 🛡️ Check 3: ที่อยู่
    elif not c_house_no: 
