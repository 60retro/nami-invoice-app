...
with st.form("invoice_request_form"):
    ...
    if submitted:
        ...
        # -------------------------------------------------------------------
        # 🔴 จุดที่ 2: สร้างตัวแปร "current_data_signature" ตรงนี้ครับ (ห้ามหาย!)
        # เอา เลขภาษี + ยอดเงิน + เบอร์โทร มาต่อกันเป็นรหัส
        current_data_signature = f"{c_tax}_{c_price}_{c_phone}"
        # -------------------------------------------------------------------

        # 3. ตรวจสอบว่ารหัสนี้ ซ้ำกับอันล่าสุดหรือไม่?
        if st.session_state['last_submitted_id'] == current_data_signature:
            st.warning("⚠️ ข้อมูลชุดนี้ถูกส่งเข้าระบบเรียบร้อยแล้ว (ป้องกันการกดซ้ำ)")
            st.stop() # หยุดทำงานทันที
        else:
